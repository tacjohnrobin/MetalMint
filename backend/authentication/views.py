...
class LogoutView(APIView):
    """View to logout user and revoke refresh token"""
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            logout(request)  # Clear session if any
            logger.info(f"User {user_email} logged out successfully")
            return Response({"message": "Logged out successfully"}, status=status.HTTP_200_OK)
        except Exception as e:
            user_email = request.user.email if request.user.is_authenticated else "unknown"
            logger.error(f"Logout failed for {user_email}: {str(e)}")
...
class SendOTPView(APIView):
    """View to send OTP to authenticated user"""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            otp.save()
            logger.info(f"OTP {otp.otp_code} generated for {user.email}")
            ...
            try:
                html_message = render_to_string(
                    "otp_email.html",
                    {
                        ...
                        "purpose": dict(OTP.PURPOSE_CHOICES)[purpose]
                    },
                )
                logger.info(f"Template rendered successfully for {user.email}")
            except Exception as template_error:
                logger.error(f"Template rendering failed: {str(template_error)}", exc_info=True)
                raise
            ...
            logger.debug(f"Preparing to send email to {user.email} with OTP {otp.otp_code}")
            mail.send(fail_silently=False)
            
            logger.info(f"OTP sent to {user.email} for purpose {purpose}")
            return Response({"message": "OTP sent successfully"}, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(f"Failed to send OTP to {user.email}: {str(e)}", exc_info=True)
            return Response({"detail": f"Failed to send OTP: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

class OTPVerificationView(APIView):
    """View to verify OTP for user actions"""
    permission_classes = [IsAuthenticated]
