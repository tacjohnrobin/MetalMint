...
export async function registerUser(
  ...
  try {
    ...
    if (responseData.access_token && responseData.refresh_token) {
      await setCookies(responseData.access_token, responseData.refresh_token);
...
export async function loginUser(
  ...
  try {
    ...
    await setCookies(responseData.access, responseData.refresh);

    console.log("✅ Login successful, sending OTP...");
    return await sendOtp(responseData.access); // Ensure OTP request is sent
  } catch (error) {
...
export async function verifyOtp(
  ...
  try {
    ...
    const purpose = data.purpose || "VA"; // Default to "login" if missing
